...
description: Design, review and change any integration with an external system — outbound calls, inbound APIs, webhooks, file transfers, identity federation, partner connectors — as a trust, availability and compliance boundary. Use at design time when anything "calls a third-party", "integrates with" or uses a "vendor API" or named provider, when a partner announces a change, and when someone asks to remove or clean up an integration that looks unused. Runs alongside secure-api-review.
---
