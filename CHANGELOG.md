...
- **Temp-directory paths are judged as paths** (REQ-USA-07, 08). `cat`, `ls`, `tail`, `mkdir`, `grep`/`rg`, `cp`/`mv` *to* a temp directory and the other `_TEMP_DATA_PROGS` no longer trip "Running … from a temporary directory".
  ...
  - **Verification fixes.** `rg` with `--pre` or `--pre-glob` in any spelling (`--pre=sh`, `--pre sh`, `--pre-g`, `--pr=`) is not a data program: ripgrep runs `COMMAND PATH` for each input. A command with any `VAR=value` prefix (`RIPGREP_CONFIG_PATH`, `LD_PRELOAD`, `LC_ALL`) on the relaxed program, or an `export`/`declare`/`set`/`source`, is not relaxed. A stdin source (`< /tmp/x`, `0< /tmp/x`, `<> /tmp/x`) is judged like a temp operand: `tee src/app.py < /tmp/x` and `cat < /tmp/x > src/app.py` are denied. A temp path that is only data or only a destination is allowed when no write of the command reaches the repository (`mv src/app.py /tmp/y`, `cp /tmp/x /tmp/y`; mv's source delete carries no content). Otherwise the reason says that temp content could reach the repository, or names the write that cannot be judged, instead of "executes code".
  - **Working directory (verification C1, Critical, present since 2.0.0).** The engine followed one `cd` at a time, so `(cd /tmp); echo x > .git/hooks/pre-commit`, `true || cd /tmp; …`, `cd /tmp && cd - && …` and `pushd /tmp && popd && …` were judged from `/tmp` and allowed. The engine now keeps a set of candidate working directories: the session's cwd, every `cd`/`pushd` target (options such as `-P` understood) resolved against every candidate, and, for `cd -`, `popd`, a bare `cd` or `pushd +N`, the repository root and HOME. Every relative write, script and the temp rule are judged from every candidate, and a denial from any one denies. Only a plain `a && b && c` chain (no subshell, `;`, `||`, `!`, keyword, substitution or heredoc) is followed exactly, so `cd src && echo x > app.py` still writes `src/app.py`. A `cd` to a path computed at run time, or a relative one while `CDPATH` may apply, makes the directory unknown: a relative write then denies with "the working directory cannot be determined". The exact chain is followed only when each segment names the program of the parsed command at its position and none is `eval`, a shell, `env`, `exec` or a similar wrapper (`cd /tmp && X=1 && eval "cd -" && …` lined the commands up wrongly); `cd -- -` goes back like `cd -`; and bash's `~-`/`~+` (the previous or current directory) as a `cd` target or a write target is treated as computed at run time.
...
- **For PILOT-59** (`cmdparse.py`, not changed here), found in the PILOT-60 review:
  - **Critical (C2):** `writes_of` ignores `uniq`'s output operand (`uniq /tmp/x src/app.py` writes `src/app.py` unjudged), and `sponge` is not a known writer (`cat /tmp/x | sponge src/app.py`);
  ...
  - temp content read through a `file://` URL (`curl -o src/app.py file:///tmp/x`) is judged only by the write's claims, as in 2.2.0 (the stdin form is fixed above);
  - a relative temp path reached through a `cd` into a temp directory (`cd /tmp && tee ../repo/x < y`) is not recognised as a temp path (the write itself is still judged from every candidate directory).
- **L3.** `run-tests.sh` refuses an `EVIDENCE_RESULTS_DIR` whose real path is inside the working tree (exit 2, before any `rm -f`; on macOS the comparison is case-insensitive). The GitLab template's tracked-results guard fails the job when `git ls-files` itself fails (L2).
