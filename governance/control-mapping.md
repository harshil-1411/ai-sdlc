...
| PW.8 Test executable code | Pre-existing tests protected in fixes (V2G-10*); `verifier` required (V2S-04*); `evidence gaps` counts only ingested JUnit/eval passes. Since 2.3.0 (ADR-0002) the test evidence is CI's signed artifact: `sign-and-gate` signs only the freshly downloaded results with the base branch's CLI and gates with `gaps --strict --only-results` on them (USA-03*, USA-04*); nothing under `validation/results/` is signed or read. | ENFORCED (partial); GUIDED; OWNER ACTION (CI: apply the 2.3.0 `ci.yml` change) | `evidence gaps`; the signed `validation-results` CI artifact of the run | Test adequacy is guided. |
