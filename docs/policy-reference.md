...
| `approval.github_identities` | not set | Org policy only (a repository policy cannot set or extend it). Maps each change creator's git email (the `created_by` of `change start`, case-insensitive) to their GitHub login, or a list of logins. `evidence approve --github-pr` refuses an approval by any of them, and records `creator_login`; the Tier 3 `tier3-same-person` rule applies the same comparison to a recorded GitHub approval. **For Tier 3, a creator with no entry makes every GitHub approval refused** (fail closed); for Tier 1 and 2 the approval is accepted with a note. Since PILOT-59 (REQ-CON-16) |
...
| Key | Default | Meaning |
| --- | --- | --- |
| `git_config_engine_ignored` | `alias.*`, `core.editor`, `core.pager`, `pager.*`, `sequence.editor`, `interactive.diffFilter`, `*tool.*.cmd` | Keys in `deny_git_config_keys` that the engine's own config check (`git-config-refused`) does **not** refuse at **global or system** scope. Why each is safe there: the engine runs only built-in subcommands (`rev-parse`, `status`, `ls-files`, `ls-tree`, `show`, `diff`, `diff-tree`, `log`, `cat-file`, `merge-base`, `rev-list`, `config`), and git never expands an alias that shadows a built-in; none of them opens an editor (`core.editor`, `sequence.editor`) or runs `add -p` (`interactive.diffFilter`), `difftool` or `mergetool` (`*tool.*.cmd`); output is captured and the engine sets `core.pager=cat`, so no pager runs (`core.pager`, `pager.*`). At local or worktree scope they are still refused: an agent can write `.git/config`, and a human would later run what it planted. Since the PILOT-60 review (M2) the exemption also requires the key to come (`git config --show-origin`) from a file the integrity monitor watches, `~/.gitconfig` or `~/.config/git/config`, or, at system scope, from a file the user cannot write (not owned by or writable by the user, parent directory likewise). The same key in an unwatched `$XDG_CONFIG_HOME/git/config` or a user-owned system gitconfig (Homebrew's `/opt/homebrew/etc/gitconfig`) is refused, because the agent could plant it there with no record. The org policy may add entries; a repository policy may only remove them. `git -c` and `git config` by an agent still deny the whole `deny_git_config_keys` list. |
...
| `approval.github_repo_roots` | not set | Org policy only (a repository policy cannot set or widen it). The checkout roots allowed to use the server gate: absolute paths, already resolved (`realpath`), compared as written. The gate is confirmed only when the checkout's real path is listed **and** its `origin` is `approval.github_repo`. Empty or unset: the gate is never confirmed. |
