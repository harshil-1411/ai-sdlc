...
| `automated_test` | `plugins/evidence-sdlc/scripts/tests/engine-tests.py` (v1 rows name the retired `gate-regression-tests.sh`) | The actual test file, so anyone can go open it and read exactly what it checks. | The file path itself, as it exists in the project today. |
