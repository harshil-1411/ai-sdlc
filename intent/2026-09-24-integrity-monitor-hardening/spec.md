...
| REQ-IMH-19 | CI's trusted gate runs `evidence verify-range --base <PR base sha> --head <PR head sha>` and fails on any problem (ADR-0004 decision 1–5): all commits including merges (`--cc`, net diff, `--root`), A/M/D/T/R against the approved plan's claims, message key and trailer, chunked secret scan with an oversize rule, audit-log prefix across the range and verification at the head, and change records verified at the head | Commit bypass, review F6–F11 | CLI tests on fixture repositories: evil merge, `commit-tree` commit without a trailer, an unclaimed deletion, a truncated audit log in a later commit, a secret added then removed, a missing state after start → each fails; a clean range passes |
| REQ-IMH-21 | `.github/workflows/ci.yml` `sign-and-gate` runs `verify-range` with the base branch's CLI and the event's SHAs, before `gaps --strict` | ADR-0004 | Content test: the workflow step exists, uses `../trusted`, and uses `github.event.pull_request.base.sha` / `head.sha` |
...
- **REQ-IMH-19** (new `lifecycle.cmd_verify_range`):
...
- **REQ-IMH-21** (`.github/workflows/ci.yml`): a new step in `sign-and-gate` before "Traceability gaps":
  `python3 ../trusted/plugins/evidence-sdlc/bin/evidence verify-range --base "${{ github.event.pull_request.base.sha }}" --head "${{ github.event.pull_request.head.sha }}"`,
  with the key available only to that step.
...
- **`ci.yml` is change-controlled.** The implementing session must be started by the human with `CHANGE_TICKET=PILOT-58`. Owner: maintainer.
